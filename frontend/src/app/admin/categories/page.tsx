...
function AdminCategoriesContent() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this category? This might affect products associated with it.')) return;
    try {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 pt-24">
      {/* Sidebar - Optional, matching orders sidebar if needed, but we'll use a simpler layout for now */}
      <aside className="w-72 glass border-r border-gray-200 p-8 flex flex-col gap-6 min-h-screen hidden md:flex">
        ...
        <div className="space-y-2 mt-4">
          ...
          <a href="/admin/categories" className="block px-4 py-3 rounded-xl bg-primary text-white shadow text-sm font-medium">Categories</a>
      ...
      <main className="flex-1 p-6 md:p-10">
        <div className="mb-8 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-primary mt-1">Manage Categories</h1>
          </div>
          <div className="flex gap-3">
            <button onClick={fetchCategories} className="bg-white border border-gray-200 px-5 py-2 rounded-lg text-sm font-medium hover:shadow-md transition-all">
              Refresh
            </button>
                  ...
                  {categories.length === 0 ? (
                    ...
                    categories.map((category) => (
                      <tr key={category.id} className="border-b border-gray-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4 text-sm font-bold text-primary">{category.name}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-text-muted mb-2">Category Name</label>
                <input 
                  type="text" 
                  value={formData.name}
